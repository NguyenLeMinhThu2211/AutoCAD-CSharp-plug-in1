...
[assembly: CommandClass(typeof(AutoCAD_CSharp_plug_in1.Quantity))]
namespace AutoCAD_CSharp_plug_in1
{
    internal class Quantity
    {
        [CommandMethod("QTK", CommandFlags.Modal)]
...
#region Tạo Khung tiêu đề
                ...
                using (BlockTableRecord Btr = new BlockTableRecord())
                {
                    ...
                    string randomBlockName = GenerateRandomName(8); // 8 là độ dài của tên
                                                                    // Kiểm tra xem tên đã tồn tại chưa
                    while (bt.Has(randomBlockName))
                    {
                        randomBlockName = GenerateRandomName(8); // Sinh tên mới nếu tên đã tồn tại
                    }
...
#region Tạo Block Attribute để lưu dữ liệu thép
                ...
                {
                    btAttName = GenerateRandomName(8);
                }
                #endregion
                ...
                if (bID != ObjectId.Null)
                {
                    using (BlockReference Br = new BlockReference(new Point3d(pt.X, pt.Y, 0), bID)) //Set location to place block
                    {
                        ...
                        SetAttToBlock(tr, Br, attBarSUMMass, Massint.ToString(), scale, false);
                    }
                }
                //--------------------------------------------------------------------------------------
                #endregion
